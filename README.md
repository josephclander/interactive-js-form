...
2. Real-time error messages were entered for all validated, text entry inputs except "name". The decision was made that "name" isn't specific enough with its validation and may just annoy the user. Real-time validation was added to the activites box. It does not start on page load, it listens for input checks and so only initiates with a correct validation on first click. This also works after an incorrect page submission.
